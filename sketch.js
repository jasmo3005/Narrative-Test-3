...
const orbMessagesRainMaster = [
  "The Orb’s magic is strong despite being asleep for many years…",
  "Embark on a strange and fruitful journey through this ancient, sleeping forest.",
  "Each button pertains to an action you can take with The Orb as your compass.",
  "The truth is fleeting. Let the Orb be your guide…"
];
...
function setup() {
  ...
  if (idleBtn) {
    idleBtn.addEventListener('click', () => {
      ...
      isMagic = false;
      showText = false;
      // If Rain Master is active, switch to new message set
      if (rainMasterActive) {
        useRainMasterMessages = true;
        orbMessageIndex = 0; // Reset to first message in new set
      }
    });
  }
  ...
  if (talkBtn) {
    talkBtn.addEventListener('click', () => {
      if (useRainMasterMessages) {
        orbMessageIndex = (orbMessageIndex + 1) % orbMessagesRainMaster.length;
      } else {
  ...
  if (rainMasterBtn) {
    rainMasterBtn.addEventListener('click', () => {
      ...
      // When Rain Master is turned on, allow switching to new messages after next idle
      if (rainMasterActive) {
        useRainMasterMessages = false; // Will be set to true after next idle
      }
    });
  }
...
let useRainMasterMessages = false;
...
function drawScene1() {
  ...
  if (showText) {
    ...
    textSize(16);
    if (useRainMasterMessages) {
      text(orbMessagesRainMaster[orbMessageIndex], width/2, height - 50);
    } else {
      text(orbMessages[orbMessageIndex], width/2, height - 50);
    }
    textSize(12);
  }
